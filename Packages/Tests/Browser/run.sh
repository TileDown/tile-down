#!/usr/bin/env bash
# Build the fixture site (normal + --drafts), serve both, run the Playwright
# browser tests, then tear everything down. Exit code propagates from the tests.
#
# Requires: a built `tiledown` (built here via `swift run`), Python Playwright,
# and a Chromium (installed via `playwright install chromium` or Homebrew).
set -euo pipefail
...
echo "Building fixture (normal + --drafts)..."
...
for port in "$normal_port" "$drafts_port"; do
  if lsof -ti "tcp:$port" >/dev/null 2>&1; then
    echo "Port $port is already in use. Free it (or set NORMAL_PORT/DRAFTS_PORT) and retry." >&2
    exit 1
  fi
done
...
echo "Serving normal on $normal_port, drafts on $drafts_port..."
