...
The rule for what each SPM target may `import`. Tiledown is a monorepo from day one with many targets in one package, and the engine (`TileKit`) and CLI product (`tiledown`) are already two targets, so this contract applies now and governs every target added to the manifest.
